$(function() {
		...
		function lunbo() {
			...
			var timeout = null;
				...
				//悬浮在窗口时停止轮播
			$(".lef .lunbo").mouseenter(function() {
					//清除定时器
					clearInterval(timer);
				})
				//鼠标移除窗口时开始轮播
			$(".lef .lunbo").mouseleave(function() {
					//设置一个3秒的自动轮播定时器
					timer = setInterval(changeTab, 4000);
				})
				//点击数字切换
			$(".lef .lunbo ul li").click(function() {
				...
				timeout = setTimeout(changgeMg, 200);
				return false;
			})
			$(".lef .lunbo ol li").click(function() {
			...
			//移动盒子和给当前索引上色
			function changgeMg() {
				var movePx = num * -412 + "px";
				$(".lef .lunbo .img").stop().animate({
					"marginLeft": movePx
				}, 500);
				$(".lef .lunbo  ul li").eq(num).addClass("bor").siblings().removeClass("bor")
				$(".lef .lunbo ol li").eq(num).addClass("ac").siblings().removeClass("ac")
			}
			//定时器函数    
			function changeTab() {
				if (num < 4) {
...
$(function() {
		//切换游戏视频
		function tabin() {
			$(".ar_top .botton_in .list_in p").click(function() {
				var i = $(this).index() - 1
				
				$(this).addClass("pst").siblings().removeClass("pst")
				$(".botton_in .warp_sp").eq(i).show().siblings().hide()
			})
		}
		tabin()
		...
		//切换图鉴分类
		function tabtu() {
			$(".araa_index_warp .warp .top  p").click(function() {
				var i = $(this).index() - 1
			
				$(this).addClass("pborder").siblings().removeClass("pborder")
				$(".araa_index_warp .warp .list_info").eq(i).show().siblings().hide()
			})
		}
		tabtu()
...
$(function() {
	...
	function ast() {
		var width_a = $("#focus_Box li").width()
		
		$("#focus_Box ul li").eq(0).css({
			"text-align": "left",
			"pading-left": "10px",
			"line-height": "35px"
		})
		$("#focus_Box ul li").eq(1).css({
			"text-align": "left",
			"pading-left": "10px",
			"line-height": "49px"
		})
		$("#focus_Box ul li").eq(3).css({
			"text-align": "right",
			"pading-right": "10px",
			"line-height": "49px"
		})
		$("#focus_Box ul  li").eq(4).css({
			"text-align": "right",
			"pading-right": "10px",
			"line-height": "35px"
		})
